...
export class SlicesComponent implements OnInit {
  ...
  dragAndDrop(event: CdkDragDrop<string[]>) {
    moveItemInArray(this.sliceData.data, event.previousIndex, event.currentIndex);
  }
  ...
  removeDevice(deviceIndex : any ,cameraIndex : any,cameraId : any,deviceId : any){
    if(this.myTimeout != "undefined"){
    this.removedCameraId = cameraId ;
    this.removedDeviceId = deviceId ;
    this.myTimeout = setTimeout(() => {
    this.sliceData.data[this.siteIndex].group[deviceIndex].cameras.splice(cameraIndex,1)
    }, 3000);
  }
  }

  removeServiceGroup(serviceIndex : any ,groupServiceIndex : any,groupId : any,serviceId : any){
    if(this.myTimeout != "undefined"){
      this.removedServiceGroupId = groupId;
      this.removedServiceId = serviceId ;
      this.myTimeout = setTimeout(() => {
      this.sliceData.data[this.siteIndex].services[serviceIndex].group.splice(groupServiceIndex,1)
      }, 3000);
    }
  }
